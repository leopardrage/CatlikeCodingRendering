#if !defined(BUMPINESS_HEIGHT_MAP_SHADER_INCLUDED)
#define BUMPINESS_HEIGHT_MAP_SHADER_INCLUDED
...
// the suffix _TexelSize is used for texture type variable to allow
// Unity to provide the texels size as fractions of U and V (for x and y values)
// and the amount of pixes (for z and w values).
...
void InitializeFragmentNormal(inout Interpolators i) {
    // Bump mapping: applied height map
    float2 deltaU = float2(_HeightMap_TexelSize.x * 0.5, 0);
    float u1 = tex2D(_HeightMap, i.uv - deltaU);
    float u2 = tex2D(_HeightMap, i.uv + deltaU);
    
    float2 deltaV = float2(_HeightMap_TexelSize.y * 0.5, 0);
    float v1 = tex2D(_HeightMap, i.uv - deltaV);
    float v2 = tex2D(_HeightMap, i.uv + deltaV);
    
    // Short formula, derived from the result of the cross product
    i.normal = float3(u1 - u2, 1, v1 - v2);
    // Equivalent with cross product
