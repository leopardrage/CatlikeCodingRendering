...
public class SemitransparentShadowsGUI : ShaderGUI {
    ...
    void DoRenderingMode() {
        ...
        if (EditorGUI.EndChangeCheck()) {
            ...
            foreach (Material material in editor.targets) {
                // The render queue is an int, but it must be always be set based
                // on Unity RenderQueue constants, because their integer value
                // might change between Unity versions and Unity performs different
                // logics based on it
                // (es.: Render.OpaqueGeometry vs Render.TransparenyGeometry draw calls)
        ...
        if (mode == RenderingMode.Fade || mode == RenderingMode.Transparent) {
            // If in Fade or Transparent mode, show a toggle to let the user choose
            // whether to use semitransparent shadows or to fallback to cutout shadows.
