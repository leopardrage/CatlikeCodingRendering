...
#if !defined(GPU_INSTANCING_SHARED)
#define GPU_INSTANCING_SHARED
...
// Declaring variables in a UNITY_INSTANCING_CBUFFER_START - UNITY_INSTANCING_CBUFFER_END
// block add them to a single instancing buffer so we can vary those variables
// without loosing the benefit of GPU instancing.
// The UNITY_INSTANCING_CBUFFER_START need a name as input parameter, which can safely
// be any name, since Unity add the UnityInstancing_ prefix to avoid name clashes
// with its own buffers.
UNITY_INSTANCING_BUFFER_START(InstanceProperties)
    // UNITY_DEFINE_INSTANCED_PROP use the platform specific syntax to declare
    // a variable in the context of a instancing buffer
    UNITY_DEFINE_INSTANCED_PROP(float4, _Color)
// This define has been added by the compiler and it's not explained in the tutorial.
// I suppose that we must have faith in the compiler and go on.
...
struct InterpolatorsVertex {
    ...
    // DYNAMICLIGHTMAP_ON is defined when we are dealing with realtime lightmaps.
    // That keyword is part of the ones listed through the multi_compile_fwdbase
    // compiler directive.
    // Stardard Shader uses a single variable for both baked and realtime lightmaps
    // UVs, but for semplicity we keep them separated.
...
struct Interpolators {
    ...
    // when cross fading between LODs we need the screen-space coordinates
    // available through the VPOS semantic. Otherwise we use the regular position.
...
UnityIndirect CreateIndirectLight (Interpolators i, float3 viewDir) {
    ...
    indirectLight.specular = 0;
        ...
        // To support realtime lightmaps we perform the same logic we used for
        // baked lightmaps, but based on the DYNAMICLIGHTMAP_ON keyword definition,
        // using unity_DynamicLightmap and unity_DynamicDirectionality as texture
        // references, the DecodeRealtimeLightmap function (since data are encoded
        // in realtime lightmaps in a different way than baked lightmaps) and using
        // a temporary variable for diffuse, so we can apply bump mapping on the specific
        // light component coming from realtime lightmaps.
        #if defined(DYNAMICLIGHTMAP_ON)
        ...
        // Apply spherical harmonics only if we are using neither baked nor realime
        // lightmaps
        #if !defined(LIGHTMAP_ON) && !defined(DYNAMICLIGHTMAP_ON)
            // UNITY_LIGHT_PROBE_PROXY_VOLUME is defined project wide in case
            // Light Probe Proxy Volumes are enabled in Graphics Settings for the
            // current tier
            #if UNITY_LIGHT_PROBE_PROXY_VOLUME
                // unity_ProbeVolumeParams.x == 1 means that the current object
                // has a LLPV
                if (unity_ProbeVolumeParams.x == 1) {
                    ...
                    // For LLVPs, spherical harmonics data are stored in linear space
                    // so we need to convert them, if we are working in gamma space.
