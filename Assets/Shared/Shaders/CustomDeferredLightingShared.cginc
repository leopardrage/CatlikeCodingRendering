#if !defined(CUSTOM_DEFERRED_LIGHTING_SHARED)
#define CUSTOM_DEFERRED_LIGHTING_SHARED
...
// This is needed for baked shadow masks (number 3 stores accumulated lighting.
// The fourth is the one containing raw sampled shadow mask values).
...
// Needed for baked shadowmasks
float GetShadowMaskAttenuation(float2 uv) {
    float attenuation = 1;
    #if defined (SHADOWS_SHADOWMASK)
        float4 mask = tex2D(_CameraGBufferTexture4, uv);
        // the dot product of the sampled raw shadowmask value with the
        // unity_OcclusionMaskSelector returns the shadowmask value for the
        // current light (it's sort of a channel selector, I suppose a
        // float4 vector with all 0s except for a 1 for the component correspoding
...
UnityLight CreateLight(float2 uv, float3 worldPos, float viewZ) {
    ...
    // We have to enforce shadow fade in case of baked shadowmasks, even if
    // there are no realtime shadows for the current light
    #if defined(SHADOWS_SHADOWMASK)
        shadowed = true;
    #endif
    ...
    if (shadowed) {
        ...
        // Since we added support for baked shadows for Mixed lights, we have to blend
        // also the shadow mask attenuation. So we replace the previous manual addition
        // with Unity's UnityMixRealtimeAndBakedShadows.
        ...
        // they are cheap.
            ...
            // The optimization shortcut is not possible when baked shadow masks are
            // enabled.
            #if !defined(SHADOWS_SHADOWMASK)
            UNITY_BRANCH
            if (shadowFade > 0.99) {
...
float4 FragmentProgram (Interpolators i) : SV_Target {
    ...
    // Since, by strange assumption, rays are in view space (camera local space)
    ...
    // to scale them till the reache the far plane
