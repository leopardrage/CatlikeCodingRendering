#if !defined(STATIC_LIGHTING_SHARED)
#define STATIC_LIGHTING_SHARED
...
Interpolators MyVertexProgram (VertexData v) {
    ...
    #if defined(LIGHTMAP_ON)
        // It seems that the lightmap uvs provided in the vertexData does provide uv
        // for the lightmap portion specific to the current mesh, but, since we are
        // dealing with a lightmap containing "sub-lightmaps" for many static-light
        // objects, we must also know where the specific "sub-lightmap" is located
        // in the lightmap texture.
        // This is the meaning of the tutorial whem it says "transformation is by object
        // not by material", meaning that each object refers to its sub-lightmap, as
        // a single texture in UVs terms (they go from 0,0 to 1,1 relative to their
        // sub-lightmap, not their entire lightmap). I wonder, however, why they
        // packed the UVs in their trasformed value in the first place, so that there
        // wouldn't be unnecessary runtime calculations, but maybe it's for the same
        // reason why they don't do that for regular UVs: to gives us more control.
        // Anyway: therefore we need its offset and size, which are
        // located in the related tiling info.
        // The lightmap is stored in the unity_Lightmap variable defined in
        // UnityShaderVariables.
        // We could have used the TRANSFORM_TEX macro, but that assumes that
        // the variable containing the tiling info (XY scale and ZW offset) is called
        // unity_Lightmap_ST, while Unity defined it as unity_LightmapST.
        // Due to this inconsistency, we have to do it manually.
