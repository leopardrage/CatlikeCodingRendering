...
#if !defined(FLAT_SCREEN_SPACE_DERIVATIVES_SHARED)
#define FLAT_SCREEN_SPACE_DERIVATIVES_SHARED
...
void InitializeFragmentNormal(inout Interpolators i) {
    // --
    // Flat triangles through screen-space partial derivatives
    // --
    // In order to render flat triangles, we have to find triangle normals
    // assuming that they might not be stored in the normal vectors within
    // VertexData.
    // To achieve this we have to find the world position of adjacent fragments.
    // ddx and ddy functions are special functions that return the partial derivative
    // along the corresponding screen-space dimension for the given position. They
    // exist in the first place to ensure mipmaps to work.
    // To understand how ddx and ddy can retrieve adjacent world space fragment position
    // (which was a total mystery to me) see this wonderful explaination:
    // https://stackoverflow.com/questions/16365385/explanation-of-dfdx#

    // Partial derivative of the world position in the screen-space X dimension:
    float3 dpdx = ddx(i.worldPos);
    // Partial derivative of the world position in the screen-space Y dimension:
    float3 dpdy = ddy(i.worldPos);

    // Cross product between them retuns a vector orthogonal to the triangle.
