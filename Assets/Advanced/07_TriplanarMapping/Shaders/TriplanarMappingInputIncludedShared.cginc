...
#if !defined(TRIPLANAR_MAPPING_INPUT_INCLUDED_SHARED)
#define TRIPLANAR_MAPPING_INPUT_INCLUDED_SHARED

#include "UnityPBSLighting.cginc"
#include "AutoLight.cginc"
#include "TriplanarMappingSurfaceShared.cginc"

#if defined(_NORMAL_MAP) || defined(_DETAIL_NORMAL_MAP) || defined(_PARALLAX_MAP)
    // Have tangent, uv1 and uv2 defined in the control point struct in the
    // tessellation shader (here are needed, but in the shadow caster shader, for
    // example, they are not).
    #define TESSELLATION_TANGENT 1
    // This is needed to separate the case when we don't need tangent space
    // (triplanar mapping) from the case when we do (all others)
...
float3 GetTangentSpaceNormal(Interpolators i) {
    float3 normal = float3(0, 0, 1);
        ...
        float3 detailNormal = UnpackScaleNormal(
            tex2D(_DetailNormalMap, UV_FUNCTION(i).zw),
            _DetailBumpScale
        );
